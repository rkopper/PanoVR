...
namespace VoiceChat
{
    public class VoiceChatRecorder : MonoBehaviour
    {
        ...
        string device = null;
        AudioClip clip = null;
        public bool transmitToggled = false;
        bool recording = false;
        bool transmitToggledAuto = false;
        ...
        public bool isTransmittingAutoDetectedVoice
        {
            get { return transmitToggledAuto; }
            set { transmitToggledAuto = value; }
        }
        ...
        public void ReadSample(bool transmit)
        {
            // Extract data
            ...
            {
                audio.clip = clip;
                //clip2 = (AudioClip)Instantiate(clip);
                //clip2 = clip;
                print("pre " + audio.time);
                checker = true;
            }
            if (!transmitToggled && checker)
            {
                //AudioClip clip2 = Instantiate(clip);
                //clip2 = clip;
                clipList.Add(clip);
                //audio.clip = clip2;
                audio.Play();
                print("post " + audio.time);
                checker = false;
            }
            ...
            if (NewSample != null && (transmit || forceTransmit > 0 || index >= autoDetectIndex))
            {
                ...
                if (index >= autoDetectIndex)
                {
                    if (forceTransmit <= 0)
                    {
                        ...
                        isTransmittingAutoDetectedVoice = false;

                    }

                    forceTransmit = forceTransmitTime;
                    isTransmittingAutoDetectedVoice = true;
                }
                else
                {
                    isTransmittingAutoDetectedVoice = false;
        ...
        public void ClickTransmit(bool state)
        {
            transmitToggled = state;
        }
